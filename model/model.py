...
from model import rnncell as rnn
...
class Model(object):
    def __init__(self, config, is_train=False):
        ...
        #Add model type by crownpku， bilstm or idcnn  #parameters for idcnn
    ...
    #v(中标|合同)总?(价|金额|额)(总计|合计|:|：)?约?为?(人民币)?(共计)?约?(?P<num>.{1,15}?)(亿|万)
    #IDCNN layer 
    ...
    def return_sentence_charrealpred(self, sess, data_manager, id_to_tag):
        """
        :param sess: session  to run the model 
        :param data: list of data
        :param id_to_tag: index to tag name
        :return: [char, real, pred]
        """
        results = []
            ...
            for batch in data_manager.iter_batch():
                ...
                for i in range(len(strings)):
                    ...
                    for char, gold, pred in zip(string, gold, pred):
                        result.append(" ".join([char, gold, pred])) #[c,g,p,c,g,p]
                    results.append(result) #[string, string] #[[c,g,p]]
            return results
        except:
            return None

    def return_sentence_pred_evaluate(self, sess, inputs, id_to_tag):
        """
        :param sess:
        :param inputs:
        :param id_to_tag:
