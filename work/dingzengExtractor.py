...
class DingZengRecord(object):
    def __init__(self, name=None, shuliang=None, jine=None, time=None, money=None, danwei=None):
        # 股东
        self.name = name #string
        # 数量
        self.shuliang = shuliang #string
        # 钱
        self.jine = jine #string
        # 月份
        self.time = time #string
        # 现金？
        self.money = money #string
        # 單位
    ...
    def to_result(self):
        ...
        return "%s\t%s\t%s\t%s\t%s" % (
            ...
            self.money if self.money is not None else '')
...
class DingZengExtractor(object):
    ...
    def extract_from_html_dir(self, html_dir_path):
        ...
        dataframe = pd.DataFrame(data=map, columns=["公告id", "增发对象", "增发数量", "增发金额", "锁定期", "认购方式"], dtype=None,
                                 copy=False)
        if os.path.exists('dz_result.csv'):
            os.remove('dz_result.csv')
        dataframe.to_csv("dz_result.csv", encoding="utf_8_sig")
    ...
    def extract(self, html_file_path):
        ...
        # self._extract_money(paragraphs)
