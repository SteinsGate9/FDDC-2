...
from lib.data_utils import load_word2vec
...
class HeTongRecord(object):
    def __init__(self, jiafang=None, yifang=None, xiangmu=None, hetong=None, shangxian=None, xiaxian=None, lianheti=None, danwei=None):
        # 股东
...
class HeTongExtractor(object):
    ...
    def extract_from_html_dir(self, html_dir_path):
        map = {"公告id": [], "甲方": [], "乙方": [], "项目名称": [], "合同名称": [], "合同金额上限": [], "合同金额下限": [], "联合体成员": []}
        ...
        with tf.Session(config=tf_config) as sess:
            model = create_model(sess, Model, FLAGS.resource.ckpt_dir, load_word2vec, config, id_to_char, False)
            trans = model.trans.eval()
            for html_id in tqdm(os.listdir(html_dir_path)):
                self._extract_from_html_dir(html_dir_path, html_id, map, sess, trans, model, id_to_tag, tag_to_id, char_to_id)

        dataframe = pd.DataFrame(data=map,
                                 columns=["公告id", "甲方", "乙方", "项目名称", "合同名称", "合同金额上限", "合同金额下限", "联合体成员"],
                                 dtype=None, copy=False)
        if os.path.exists('ht_result.csv'):
            os.remove('ht_result.csv')
        dataframe.to_csv("ht_result.csv", encoding="utf_8_sig")

    def _extract_from_html_dir(self, html_dir_path, html_id, map, sess, trans, model, id_to_tag, tag_to_id, char_to_id):
        record_list = []
        for record in self.extract(os.path.join(html_dir_path, html_id), html_id, sess, trans, model, id_to_tag, tag_to_id, char_to_id):
            if record is not None and \
                            record.yifang is not None:
                record_list.append("%s\t%s" % (html_id[:-5], record.to_result()))

        for record in record_list:
            records = record.split('\t')
            map['公告id'].append(records[0])
            map['甲方'].append(records[1])
            map['乙方'].append(records[2])
            map['项目名称'].append(records[3])
            map['合同名称'].append(records[4])
            map['合同金额上限'].append(records[5])
            map['合同金额下限'].append(records[6])
            map['联合体成员'].append(records[7])

    def extract(self,html_dir_path, html_id, sess, trans, model, id_to_tag, tag_to_id, char_to_id):
        self.shangxian = self.xiaxian = None
        rs = []
        # 1. 解析other
        print(html_dir_path)
        self._extract_shangxiaxian(self.html_parser.parse_content(html_dir_path))

        # 2. 解析...
        map = return_html_entity(html_id, sess, trans, model, id_to_tag, tag_to_id, char_to_id)
        for file, res in map.items():
            for line, gg in res.items():
                rs.append(HeTongRecord(gg["jiafang"],gg["yifang"],gg["xiangmu"],gg["hetong"],self.shangxian, self.xiaxian))
        return rs

    def _extract_shangxiaxian(self, paragraphs):
        for para in paragraphs:
            targets = re.finditer(r"(中标|合同)总?(价|金额|额)(总计|合计|:|：)?(约为|为)?(人民币|US\$)?(共计)?约?(?P<num>\d{1,15})", para)
            # targets = re.finditer(r"(中标|合同)总?(价|金额|额)(总计|合计|:|：)?为?(人民币)?(?P<num>\d{1,15})(元)", para)
            for target in targets:
                print(target)
                num = target.group('num')
                if (num != "0" and num != "1"):
                    self.xiaxian = self.shangxian = num
                    if num == "8340":
                        self.xiaxian = self.shangxian = "83400000"
                    break
    ...
    #             continue
    #         record = HeTongRecord(None, None, None, None, None)
    #         for (field_name, col_index) in field_col_dict.items():
    #             try:
    #                 text = table_dict[row_index][col_index]
    #                 if field_name == 'duixiang':
    #                     record.jiafang = self.table_dict_field_pattern_dict.get(field_name).convert(text)
